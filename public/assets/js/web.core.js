...
function WebPagesManagerInterface() {
    ...
    this.push = function (url, title, history_replace) {
        ...
        if (!history_replace) {
            History.pushState(null, title ? title : ENV.hotelname, "/" + url);
        } else {
            History.replaceState(null, title ? title : ENV.hotelname, "/" + url);
        }
    };
    ...
    this.load = function (url, data, scroll, callback, history_push, history_replace) {
        ...
        if (!url.match(/^\/hotel/)) {
            ...
            $.ajax({
                ...
                var pageTitle = result.page + ' - ' + ENV.hotelname;
            ...
            self.push(url, "Hotel - " + ENV.hotelname, false);
        }
    };
}
